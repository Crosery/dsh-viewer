...
一条序列只有在本插件需要的每个包都发布到它上面时才装得上，而有两条不是。`0.1.5-alpha.2` 发布的 `@deepseek-ai/dsh-tools` 要求 `@deepseek-ai/dsh-user-approval@^0.1.5-alpha.2`，该 tag 从未发过这个版本；`0.1.2-alpha.5` 是同一个形状——它的 `dsh-tools` 要 `dsh-user-approval@^0.1.2-alpha.5`，而后者只有 `0.1.2-rc.1`，那个又要求 `dsh-agent@^0.1.2-rc.1`。两条序列都根本解析不出来。这是上游的状态，不是本插件的声明；定时 job 会继续如实报告。
